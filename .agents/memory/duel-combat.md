...
**How to apply:** Keep duel UI and state in the combat command path, validate both teams before starting, assign ATK to the highest-power card, then HP to the highest-health remaining card, then SPD to the last card, disable a player's entire choice row after selection, serialize both pick handlers before resolving a round, keep the requested separator/newline layout, show only the latest log, and release both active-player locks on every end state.