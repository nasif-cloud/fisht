...
**How to apply:** Add future progression sources through the shared XP helper instead of modifying `User.xp` directly. Keep level-up Beli and Meat rewards in that helper, reconcile older profiles missing Meat, and route the plain-text notice to the originating result message in chat or to DMs when enabled.