...
independent 1–3 Gem roll per Crate. The shared slash entry point is
`/open crate [amount]`; prefix users can use `op crate [amount]`. The opening
flow matches Chests, including `all`, a three-second opening message,
pluralized wording, and a white result embed.

Chests use the same slash entry point as `/open chest [amount]`; their existing
prefix command remains `op chest [amount]`.
...
paths, keep Crate consumption plus Beli/Gem increments in one conditional
MongoDB update, and add future container slash flows under `/open`.