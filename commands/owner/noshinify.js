// Owner-only prefix command: remove the shiny status from every copy of a user's card
// Usage: op noshinify @user [cardname]
// The card can be found by its name or alias.
//
// Shiny is stored on the character's card entry, so setting `shiny: false`
// reverts all copies of that character at once.
...
module.exports = {
  ...
  async execute(message) {
    ...
    // Example:
    // "op noshinify @user Monkey D. Luffy"
    ...
    // Update every matching entry so all copies lose their shiny status,
    // including duplicate legacy entries for the same character if present.
